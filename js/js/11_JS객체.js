...
document.getElementById("btn1").addEventListener("click", function(){
    ...
    // {} 객체 생성 및 다루기
    //  ** 중요 **
    // JS 객체의 key는 무조건 string(묵시적으로도 string -> 안 써도 string)
    // -> key 작성시, "",'',(따옴표 X)든 모두 string 인식
    ...
    const product = {
        ...
        'information' : function(){ // 객체의 기능
            ...
            // 해당 객체의 변수(key)가 아닌,
            // 전역 변수가 선택됨
            // 왜? 같은 객체의 변수(key)를 선택하는 방법이 별도 존재하여
    ...
    // for(let key in 객체명) : 매 바퀴마다 객체의 key를 하나씩 얻어와서 key변수에 저장시켜두는 역할
...
document.getElementById("btn2").addEventListener("click", function(){
    ...
    stdList.push(new Student(1,2,'이현경'));
    stdList.push(new Student(2,2,'김현경'));
    stdList.push(new Student(3,2,'조현경'));
    stdList.push(new Student(4,2,'박현경'));
